...
make_tag <- function(df) {
  df %>%
    mutate(tag = paste0("ge", sigma2_ge, 
                        "_edistr_", e_distr,
                        "_eicc", e_icc,
                        "_e", sigma2_e, 
                        "_g", sigma2_g, 
...
base_t1e_scenario_df <- expand_grid(
  sigma2_ge = 0,
  e_distr = c("normal", "gamma", "gamma2"),
  e_icc = 1,
  ...
  filter(sigma2_e == sigma2_g) %>%
  make_tag()

ge_nl_t1e_scenario_df <- base_t1e_scenario_df %>%
  filter(sigma2_e == sigma2_g,
         e_distr == "gamma") %>%
  select(-sigma2_ge, -nl_e) %>%
  expand_grid(sigma2_ge = c(0, 0.1),
              nl_e = c(FALSE, TRUE)) %>%
  make_tag()

full_t1e_scenario_df <- bind_rows(list(
  base = base_t1e_scenario_df, 
  ge_nl = ge_nl_t1e_scenario_df
), .id = "subset")

full_t1e_scenario_df %>%
  distinct() %>%
  write_csv("../data/processed/simulations/t1e_scenarios.csv")
write(full_t1e_scenario_df$tag, "../data/processed/simulations/t1e_scenario_tags.txt")
...
edistr_power_scenario_df <- base_power_scenario_df %>%
  filter(sigma2_e == 0.1,
         sigma2_g == 0.1) %>%
  select(-e_distr) %>%
  expand_grid(e_distr = c("normal", "gamma", "gamma2", 
...
t1e_plt <- t1e_scenario_res_df %>%
  ...
  facet_grid(rows = vars(sigma2_e), cols = vars(sigma2_g),
             labeller = label_bquote(
               rows = "Variance explained by E: " * .(as.character(sigma2_e)),
               cols = "Variance explained by G: " * .(as.character(sigma2_g)),
             )) +
...
t1e_gamma_ge_nl_plt <- t1e_scenario_res_df %>%
  ...
  geom_errorbar(aes(ymin = l95, ymax = u95), width = 0.2, color = "gray") +
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "gray") +
  facet_grid(rows = vars(nl_e), cols = vars(sigma2_ge),
             labeller = label_bquote(
               rows = "Nonlinear E effect: " * .(as.character(nl_e)),
               cols = "G-E correlation (" * Var["G->E"] * "): " * .(sigma2_ge)
             )) +
  coord_cartesian(ylim = c(0, 0.2)) +
  labs(x = "",
       y = "False positive rate",
...
t1e_estimates_plt <- t1e_scenario_res_df %>%
  filter(sigma2_g == 0.1) %>%
  select(sigma2_ge, nl_e, sigma2_e, sigma2_g, pgs_type,
...
primary_pwr_plt <- power_scenario_res_df %>%
  ...
  mutate(se = sqrt(prop_sig * (1 - prop_sig) / n_sim),
         l95 = prop_sig - 1.96 * se,
         u95 = prop_sig + 1.96 * se) %>%
  ggplot(aes(x = sigma2_gxe, y = prop_sig, 
  ...
  facet_grid(rows = vars(sigma2_e), cols = vars(sigma2_g),
             labeller = label_bquote(
               rows = Var["E,tot"] * ": " * .(sigma2_e),
               cols = Var["G,tot"] * ": " * .(sigma2_g)
             )) +
  scale_color_discrete(name = "PGS type") +
...
primary_effect_plt <- power_scenario_res_df %>%
  filter(subset == "base") %>%
  select(sigma2_gxe, sigma2_e, sigma2_g, pgs_type, 
         mean_effect, sd_effect, mean_effect_main, sd_effect_main) %>%
  pivot_longer(cols = c(mean_effect, mean_effect_main), 
               names_to = "effect_type", 
               values_to = "mean_value") %>%
  ...
  mutate(linetype = if_else(effect_type == "mean_effect", "Interaction", "Main"),
         ci_lower = mean_value - 1.96 * sd_value,
         ci_upper = mean_value + 1.96 * sd_value) %>%
  ggplot(aes(x = sigma2_gxe, y = mean_value, 
...
e_distributions <- c("normal", "gamma", "sigma2_e")
e_distributions_clean <- c("Std. normal", "Gamma", "Normal (mean=1)")
...
t1e_normal_plt <- t1e_scenario_res_df %>%
  ...
  geom_errorbar(aes(ymin = l95, ymax = u95), width = 0.2, color = "gray") +
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "gray") +
  facet_grid(rows = vars(sigma2_e),
             labeller = label_bquote(
               rows = "Main effects of E & G: " * .(as.character(sigma2_e))
             )) +
  coord_cartesian(ylim = c(0, 0.2)) +
  labs(x = "",
       y = "False positive rate")
